...
import { PutObjectCommand, GetObjectCommand, ListBucketsCommand, CreateBucketCommand } from '@aws-sdk/client-s3';
...
export class ImageService {
    ...
    constructor(
        ...
        if (!this.localstackService.s3Client || !this.localstackService.bucketName) {
            throw new InternalServerErrorException('AwsService is not properly initialized');
        }
    ...
    async saveImage(data: SaveImageDto): Promise<SaveImageResponse> {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                this.logger.log(`Successfully uploaded image to S3: ${bucketName}/${s3Key}`);
                break;
            } catch (error) {
                attempts++;
                this.logger.warn(`S3 Upload Error (Attempt ${attempts}/${maxAttempts} for ${bucketName}/${s3Key}):`, error);
    ...
    private async ensureBucketExists(bucketName: string): Promise<void> {
        try {
            const listBuckets = await this.localstackService.s3Client.send(new ListBucketsCommand({}));
            const bucketExists = listBuckets.Buckets?.some((b) => b.Name === bucketName);
            if (!bucketExists) {
                this.logger.log(`Creating bucket ${bucketName}`);
                await this.localstackService.s3Client.send(new CreateBucketCommand({ Bucket: bucketName }));
                this.logger.log(`Bucket ${bucketName} created successfully`);
            }
        } catch (error) {
            this.logger.error(`Failed to verify or create bucket ${bucketName}:`, error);
            throw ImageMessageException.S3UploadFailed();
        }
    }
    ...
    async getImageByQrCodeId(qrCodeId: string): Promise<{ base64: string }> {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                attempts++;
                this.logger.warn(`S3 GetObject Error (Attempt ${attempts}/${maxAttempts} for ${imageMetadata.s3Key}):`, error);
                if (attempts === maxAttempts) {
                    this.logger.error(`S3 GetObject Error after ${maxAttempts} retries:`, error);
