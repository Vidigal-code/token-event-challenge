...
export class SaveImageDto {
    ...
    @IsString()
    @IsNotEmpty()
    @Matches(/^data:image\/\w+;base64,/, { message: 'Invalid base64 image format' })
    base64: string;
    ...
    @IsString()
    @IsNotEmpty()
    @Matches(/^\d{4}-\d{2}-\d{2}$/, { message: 'Invalid date format (expected YYYY-MM-DD)' })
    date: string;
    ...
    @IsString()
    @IsNotEmpty()
    @Matches(/^\d{2}:\d{2}:\d{2}$/, { message: 'Invalid time format (expected HH:mm:ss)' })
    time: string;
